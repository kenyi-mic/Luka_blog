...
function Post({post}:Props) {
  ...
  return (
    <main>
        <Header/>
        <Image className="w-full h-40 object-cover" src={urlFor(post.mainImage).url()!} alt={post.title}  width="100rem" height="10rem" />
           ...
           content={post.body}
             serializers = {{
                 ...
                 link:({href, children}: any)=>( 
                     <a href={href} className="text-amber-700 hover:underline">{children}</a>
                 )
             }}
             />
       </div>
    </article>
    <hr className="max-w-lg my-5 mx-auto border border-blue-600"/>
    {submitted ?(
        <div className="flex flex-col p-10 my-10 bg-gradient-to-br from-blue-600 to-purple-500 rounded  text-white max-w-2xl mx-auto">
...
<form onSubmit={handleSubmit(onSubmit)} className="flex flex-col p-5 max-w-2xl mx-auto mb-10">
        ...
        <input type="submit" className="shadow bg-gradient-to-br from-blue-600 to-purple-600 hover:bg-blue-400 focus:shadow-outline focus:outline-none text-gray-200 font-bold py-2 px-4 rounded cursor-pointer" />
    </form>
    )}
    {/* comments */}
    <div  className="flex flex-col p-10 shadow-blue-500 shadow space-y-2 max-w-2xl mx-auto my-10 ">
        <h3 className="text-4xl ">Comments</h3>
        <hr  className="pb-2"/>
        {post.comments.map((comment) => ( 
            <div key={comment._id} >
                <p >
                   <span className="text-amber-600 font-mono">{comment.name} - </span> {comment.comment}
                </p>
            </div>
        ))}
    </div>
