        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 p-2 md:p-6">
          {books.map((book) => ( 
            <Link key={book._id} href={`/book/${book.slug.current}`}>
              <div className="border border-6 border-cyan-700 overflow-hidden  my-10 group cursor-pointer">
                <img className="group-hover:scale-105 transition-transform  duration-1000 ease-in-out w-80 object-cover" src={urlFor(book.mainImage).url()!} alt={book.title} />
                <div className="flex flex-col-1 justify-between p-2">
                  ...
                  <img className="w-12 h-12 rounded-full " src={urlFor(book.author.image).url()!} alt={book.author.name} />
