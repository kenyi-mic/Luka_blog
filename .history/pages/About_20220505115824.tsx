        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 p-2 md:p-6">
          {books.map((book) => ( 
            <Link key={book._id} href={`/book/${book.slug.current}`}>
              <div className="border shadow-purple-200 shadow-lg overflow-hidden  my-10 group cursor-pointer">
                <img className="group-hover:scale-105 transition-transform  duration-1000 ease-in-out w-full h-110  object-contain" src={urlFor(book.mainImage).url()!} alt={book.title} />
