        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6 p-2 md:p-6 ">
          {books.map((book) => ( 
            <Link key={book._id} href={`/book/${book.slug.current}`}>
              <div className="">
                <img className="h-80 w-full" src={urlFor(book.mainImage).url()!} alt={book.title} />
                <div className="flex justify-evenly">
                  <div className="py-4 ">
                    <p className="font-medium text-indigo-600">{book.title}</p>
                    <p className="text-sm font-extralight text-gray-600 my-3">{book.description.length > 50 ? book.description.slice(0, 40) + '...' : book.description }</p>
                    <Link href={book.url} >
                    <p className="hover:text-yellow-600 text-indigo-500 bg-gray-200 w-20 px-2 py-1 rounded-full">{book.link}</p>
                    </Link>
